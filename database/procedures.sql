...
CREATE OR REPLACE PROCEDURE proc_modifier_membre(
    ...
    p_adresse VARCHAR2 DEFAULT NULL,
    p_email VARCHAR2 DEFAULT NULL,
    p_date_naissance DATE DEFAULT NULL,
    p_statut VARCHAR2 DEFAULT NULL
) IS
  v_count NUMBER;
BEGIN
  -- Vérifier que le membre existe
  SELECT COUNT(*) INTO v_count FROM membres WHERE membre_id = p_membre_id;
  IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20030, 'Membre non trouvé');
  END IF;

  -- Vérifier email unique si changement d'email
  IF p_email IS NOT NULL THEN
    SELECT COUNT(*) INTO v_count
    FROM membres
    WHERE email = p_email AND membre_id != p_membre_id;
...
-- Modifier infos coach
CREATE OR REPLACE PROCEDURE proc_modifier_coach(
    p_coach_id NUMBER,
    p_nom VARCHAR2 DEFAULT NULL,
    p_prenom VARCHAR2 DEFAULT NULL,
    p_telephone VARCHAR2 DEFAULT NULL,
    p_specialites VARCHAR2 DEFAULT NULL,
    p_email VARCHAR2 DEFAULT NULL
) IS
  v_count NUMBER;
BEGIN
  -- Vérifier que le coach existe
  SELECT COUNT(*) INTO v_count FROM coachs WHERE coach_id = p_coach_id;
  IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20031, 'Coach non trouvé');
  END IF;
...
-- Annuler paiement
CREATE OR REPLACE PROCEDURE proc_annuler_paiement(paiement_id NUMBER) IS
  v_souscription_id NUMBER;
BEGIN
  SELECT souscription_id INTO v_souscription_id FROM paiements WHERE paiement_id = paiement_id;

  UPDATE paiements SET statut_paiement = 'REFUSE' WHERE paiement_id = paiement_id;

  -- Si plus aucun paiement valide, repasser souscription à EN_ATTENTE
  IF (SELECT COUNT(*) FROM paiements WHERE souscription_id = v_souscription_id AND statut_paiement = 'VALIDE') = 0 THEN
    UPDATE souscriptions SET statut = 'EN_ATTENTE' WHERE souscription_id = v_souscription_id;
  END IF;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM sessions WHERE membre_id = p_membre_id AND ((date_heure_debut < p_date_fin AND date_heure_fin > p_date_debut))) THEN
...
CREATE OR REPLACE PROCEDURE proc_enregistrer_acces(p_membre_id NUMBER) IS
  ...
  v_acces_en_cours NUMBER;
BEGIN
  -- Vérifier abonnement valide
  IF NOT func_verifier_abonnement_valide(p_membre_id) THEN
    RAISE_APPLICATION_ERROR(-20020, 'Accès refusé : Abonnement expiré ou inactif');
  END IF;

  -- Vérifier qu'il n'y a pas déjà un accès EN_COURS
  SELECT COUNT(*) INTO v_acces_en_cours
  FROM acces
  WHERE membre_id = p_membre_id AND statut_acces = 'EN_COURS';

  IF v_acces_en_cours > 0 THEN
    RAISE_APPLICATION_ERROR(-20022, 'Vous avez déjà un accès en cours. Veuillez enregistrer la sortie d''abord.');
  END IF;

  -- Vérifier les horaires d'ouverture (6h-23h)
  v_heure_actuelle := TO_NUMBER(TO_CHAR(SYSTIMESTAMP, 'HH24'));
  
  IF v_heure_actuelle < 6 OR v_heure_actuelle >= 23 THEN
    RAISE_APPLICATION_ERROR(-20021, 'La salle est fermée. Horaires : 6h-23h');
  END IF;
...
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20030, 'Cet accès n''existe pas');
  END IF;
  
  -- Vérifier le statut actuel
  SELECT statut_acces INTO v_statut
  FROM acces
  WHERE acces_id = p_acces_id;
  
  IF v_statut != 'EN_COURS' THEN
    RAISE_APPLICATION_ERROR(-20030, 'Cet accès est déjà terminé ou n''existe pas');
  END IF;
...
-- Enregistrer sortie (alias de proc_terminer_acces)
CREATE OR REPLACE PROCEDURE proc_enregistrer_sortie(p_acces_id NUMBER) IS
BEGIN
  proc_terminer_acces(p_acces_id);
END;
/
